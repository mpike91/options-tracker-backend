...
public class CboeService {
    ...
    // GetWeeklyStocksAsync()
    public async Task<List<WeeklyStockDTO>> GetWeeklyStocksAsync()
    {
        var stocks = await _db.WeeklyStocks.Select(s => new WeeklyStockDTO
        {
            Symbol = s.Symbol,
            Name = s.Name,
            ...
            using (var reader = new StreamReader(csvStream)) {
                using (var csv = new CsvReader(reader, CultureInfo.InvariantCulture)) {
                    ...
                    while (csv.Read()) {
                        string? symbol = null;
                        string? name = null;
                        if (csv.TryGetField(0, out symbol) && csv.TryGetField(1, out name) &&
                            !string.IsNullOrWhiteSpace(symbol) && !string.IsNullOrWhiteSpace(name) &&
                            !csv.TryGetField<string?>(2, out _)) {  // Ensure no third field (discard out)

                            // Validate symbol-like (uppercase alphanum, short length) and name has letters
                            if (symbol != null && symbol.Length <= 10 && symbol.All(c => char.IsUpper(c) || char.IsDigit(c)) &&
                                name != null && name.Any(char.IsLetter)) {
